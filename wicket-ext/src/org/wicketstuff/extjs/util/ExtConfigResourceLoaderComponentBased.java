 ...
 *
 */
public class ExtConfigResourceLoaderComponentBased
{

	public static final WeakHashMap<String, Config> cache = new WeakHashMap<String, Config>();
	...
	public Config loadConfigResource( final Component component, final String className ) {
		...
		return new Config(config); // <-- always return the object copy
	}
			...
			if (props != null)
			{
				...
				while( i.hasNext() ) {
					...
					if( name.startsWith(keyPrefix + ".") ) {
						...
						config.putIfNotExists(property, typedValue(props.getString(name)));
					}
				}
			}
		}
	...
	protected void loadStringResourceOnParents(Class clazz, final String keyPrefix, final Locale locale, final String style, final Config config )
	{
		if (clazz == null)
		{
			return;
		...
		while (true)
		{
			loadStringResource(clazz, keyPrefix, locale, style, config);
	...
	protected void loadStringResource(final Component component, final String key, final Config config )
	{
		...
		String componentName = getClassName(component.getClass());
		...
		for (int i = searchStack.size() - 1; i >= 0; i--)
		{
			...
			String absoluteKey = componentName + "." + key;
			if( i == 0 && component.getClass().equals(clazz)) {
				 ...
				 */
				loadStringResource( Application.get().getClass(), absoluteKey, locale, style, config);
			}

			if( component.getClass().equals(clazz) ) {
				loadStringResourceOnParents(clazz, key, locale, style, config);
			}
			else {
				loadStringResource(clazz, absoluteKey, locale, style, config);
			}
		}
